...
GoSub, TrayMenu    ;;∙------∙Builds the tray menu.
...
;;∙------∙Close Button (X) / Window Icon Button.
...
;;∙------∙Hide/Minimize Button (-) / Window Icon Button.
...
Gui, Converter:Font, s10 q5, Segoe UI
Gui, Converter:Add, Tab2, x15 y50 w%TguiW% h0 vConverterTabs gTabChanged AltSubmit -Wrap, Intro|Length|Weight|Volume|Area|Temp|Speed|Time|Storage|Currency|Pressure|Energy|Power|Fuel|Data|Angle|Torque|Force|Accel|    ;;∙------∙Creates an Intro tab and 18 converter tabs.
...
Gui, Converter:Add, Edit, x55 y170 w305 h30 vInputValue gAutoConvert Number    ;;∙------∙User input field.
...
Gui, Converter:Add, Text, x55 y268 w305 h22 vLengthFullResult BackgroundTrans    ;;∙------∙Shows full precision value.
...
;;∙================================================∙
;;∙======∙SHOW GUI & INITIALIZE∙====================∙
GuiControl, Converter:Choose, ConverterTabs, % defaultTab    ;;∙------∙Sets the starting tab.
...
GetSecondsFactor(unitName) {
    If (unitName = "Milliseconds")
        Return 0.001    ;;∙------∙1 ms = 0.001 seconds.
    If (unitName = "Seconds")
        Return 1.0    ;;∙------∙Base unit.
    If (unitName = "Minutes")
        Return 60.0    ;;∙------∙1 minute = 60 seconds.
    If (unitName = "Hours")
        Return 3600.0    ;;∙------∙1 hour = 3600 seconds.
    If (unitName = "Days")
        Return 86400.0    ;;∙------∙1 day = 86400 seconds.
    If (unitName = "Weeks")
        Return 604800.0    ;;∙------∙1 week = 604800 seconds.
    If (unitName = "Years")
        Return 31557600.0    ;;∙------∙1 year (365.25 days) = 31,557,600 seconds.
}
...
GetCurrencyCode(fullName) {
    Return SubStr(fullName, 1, 3)    ;;∙------∙Extracts 3-letter code from display name (e.g., "USD").
}
...
CurrencyAutoConvert:
    ...
    Else
        GuiControl, Converter:, CurrencyResultValue, % Format("{:.4f}", result)    ;;∙------∙Currency typically shown to 4 decimals.
...
ConvertFuel(value, fromUnitName, toUnitName) {
    ...
    Else If (fromUnitName = "L/100km")
        mpgUS := 235.214583 / value    ;;∙------∙Conversion formula.
...
TabChanged:
    ...
    If (ConverterTabs = 10 && lastApiCheck = "") {    ;;∙------∙Check if Currency tab is opened without rates.
        GoSub, CurrencyFetchRates    ;;∙------∙Fetch rates on-demand.
    }
Return
...
WM_LBUTTONDOWNdrag() {
   PostMessage, 0x00A1, 2, 0    ;;∙------∙Send WM_NCLBUTTONDOWN to title bar area (allows drag).
}
Return
...
FileGetTime, currentModTime, %A_ScriptFullPath%
    if  (oldModTime = currentModTime) Or (oldModTime = "")
        Return
    Soundbeep, 1700, 100    ;;∙------∙Audible notification that script changed.
Reload    ;;∙------∙Reload script to apply changes.
