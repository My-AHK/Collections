...
;;∙------∙Window Settings∙-------------------------------------------∙
defaultOrientation := "Horizontal"    ;;∙------∙Default ruler orientation (Horizontal or Vertical).
iconThemeColor := "0"    ;;∙------∙0 = white icon, 1 = black icon (Transparency & Orientation buttons).
rulerLength := 800    ;;∙------∙Length of ruler in pixels.
rulerThickness := 70    ;;∙------∙Thickness/width of ruler in pixels.
windowTransparency := 255    ;;∙------∙Initial window transparency (255 = opaque, 0 = invisible).
transparencyLevel := 150    ;;∙------∙Transparency level when toggled on (0-254).
rulerOnTop := true    ;;∙------∙Keep ruler window always on top of other windows.
guiX := 100    ;;∙------∙Initial X position of ruler window on screen (pixels from left).
guiY := 100    ;;∙------∙Initial Y position of ruler window on screen (pixels from top).
pixelsPerInch := 96  	;;∙------∙Screen resolution: 96 pixels = 1 inch (typical screen).
    		;;∙------∙Alternative: 300 pixels = 1 inch (typical print quality).

;;∙------∙Colors & Appearance∙--------------------------------------∙
gBackground := "Black"    ;;∙------∙Background color of ruler window.
outerBorder := "Magenta"    ;;∙------∙Color of outer border around the entire window.
tickColor := "Yellow"    ;;∙------∙Color of tick marks on the ruler.
tickLabelColor := "Aqua"    ;;∙------∙Color of numbers next to tick marks.
measureTextColor := "Lime"    ;;∙------∙Color of measurement display text.
titleBarColor := "Azure"    ;;∙------∙Color of title text.
rulerOutlineColor := "Orange"    ;;∙------∙Color of 1px outline around the ruler bar.

;;∙================================================∙
;;∙======∙GLOBAL VARIABLES∙========================∙
ScriptID := "Desktop_Ruler"    ;;∙------∙Unique identifier for ruler window.
orientation := defaultOrientation    ;;∙------∙Current orientation (Horizontal or Vertical).
isMeasuring := false    ;;∙------∙Flag indicating if measuring is in progress.
measureStartX := 0    ;;∙------∙Starting X coordinate for measurement.
measureStartY := 0    ;;∙------∙Starting Y coordinate for measurement.
measureEndX := 0    ;;∙------∙Ending X coordinate for measurement.
measureEndY := 0    ;;∙------∙Ending Y coordinate for measurement.
measureClickCount := 0    ;;∙------∙Number of clicks recorded (0, 1, or 2).
measureModeActive := false    ;;∙------∙Tracks if Ctrl+Alt+M has been pressed to activate measuring.
blockedModeActive := false    ;;∙------∙Tracks if Ctrl+Alt+B was used to activate blocked measuring mode.
measureBlockActive := false    ;;∙------∙Tracks if cursor movement is currently blocked.
blockedTextHwnd := 0    ;;∙------∙Windows handle for "BLOCKED" text control on GUI.
...
Return
;;∙------------------------------------∙
^!m::    ;;∙------∙🔥∙(Ctrl+Alt+M) - Normal measuring mode.
    GoSub, ActivateMeasuring
Return
;;∙------------------------------------∙
^!b::    ;;∙------∙🔥∙(Ctrl+Alt+B) - Blocked measuring mode.
    GoSub, ActivateMeasuringBlocked
Return
;;∙------------------------------------∙
...
;;∙================================================∙
;;∙======∙BLOCKED MEASURING MODE∙================∙
ActivateMeasuringBlocked:
    measureModeActive := true
    blockedModeActive := true
    measureClickCount := 0
    UpdateMeasureDisplayText("awaiting_start")
    ToolTip, BLOCKED Measuring mode active. Press Ctrl+Alt+LClick to set start point.
    SetTimer, RemoveToolTip, -2500
Return
...
SetMeasurePoint:
    If (!measureModeActive) {
        ToolTip, Press Ctrl+Alt+M (or Ctrl+Alt+B for blocked mode) first.
        SetTimer, RemoveToolTip, -2500
        Return
    }
    If (measureClickCount = 0) {
        ...
        measureClickCount := 1
        ;;∙------∙Activate blocking if in blocked mode.
        If (blockedModeActive) {
            measureBlockActive := true
            If (orientation = "Horizontal") {
                ;;∙------∙Horizontal ruler → lock vertical movement.
                ClipCursorVertical()
                ToolTip, Vertical movement BLOCKED - Click again to measure
            } Else {
                ;;∙------∙Vertical ruler → lock horizontal movement.
                ClipCursorHorizontal()
                ToolTip, Horizontal movement BLOCKED - Click again to measure
            }
            SetTimer, RemoveToolTip, -2000
        }
        UpdateMeasureDisplayText("awaiting_stop")
        SetTimer, LiveAngleTooltip, 50
        ...
        If (measureBlockActive) {
            ClipCursor()
            measureBlockActive := false
        }
        SetTimer, LiveAngleTooltip, Off
        ToolTip
        ...
        blockedModeActive := false
        measureClickCount := 0
        ;;∙------∙Remove "BLOCKED" text from GUI.
        If (blockedTextHwnd)
            GuiControl, Ruler: Hide, %blockedTextHwnd%
    }
Return
...
;;∙================================================∙
;;∙======∙CLIPCURSOR FUNCTIONS∙====================∙
ClipCursorHorizontal() {
    CoordMode, Mouse, Screen
    MouseGetPos, X, Y
    VarSetCapacity(RECT, 16, 0)
    ;;∙------∙Lock X axis at current position.
    NumPut(X, RECT, 0, "Int")
    NumPut(0, RECT, 4, "Int")
...
ClipCursorVertical() {
    ...
    MouseGetPos, X, Y
    VarSetCapacity(RECT, 16, 0)
    ;;∙------∙Lock Y axis at current position.
    NumPut(0, RECT, 0, "Int")
    NumPut(Y, RECT, 4, "Int")
...
BuildRulerGUI:
    ...
    If (orientation = "Horizontal") {
        ...
        ;;∙------∙"BLOCKED" text.
        Gui, Ruler:Font, s11 cRed Bold, Arial
        Gui, Ruler:Add, Text, x365 y15 h20 Left BackgroundTrans vBlockedText Hidden, BLOCKED
    } Else {
        ...
        ;;∙------∙"BLOCKED" text.
        Gui, Ruler:Font, s11 cRed Bold, Arial
        Gui, Ruler:Add, Text, x20 y240 w60 Left BackgroundTrans vBlockedText Hidden, B`nL`nO`nC`nK`nE`nD
    }
    ...
    If (orientation = "Horizontal") {
        ...
        Gui, Ruler:Add, Text, x7 y130 w60 h150 vMeasureDisplay Left BackgroundTrans, % GetInitialDisplayText()
    }

    ;;∙------∙Store blocked text handle for later showing/hiding.
    GuiControlGet, blockedTextHwnd, Hwnd, BlockedText
...
GetInitialDisplayText() {
    global orientation
    If (orientation = "Horizontal")
        Return "Press (Ctrl+Alt+M) or (Ctrl+Alt+B) - Locks cursor to H or V axis only"
    Else
        Return "Press`nCtrl+Alt`n     +`nM or B`n    To`nMeasure`n`nB - Locks`ncursor to`n H or V`naxis only"
}
;;∙------------------------------------∙
UpdateMeasureDisplayText(state) {
    global orientation, blockedModeActive, blockedTextHwnd
    If (state = "idle") {
        If (orientation = "Horizontal")
            GuiControl, Ruler:, MeasureDisplay, Press (Ctrl+Alt+M) or (Ctrl+Alt+B) - Locks cursor to H or V axis only
        Else
            GuiControl, Ruler:, MeasureDisplay, Press`nCtrl+Alt`n     +`nM or B`n    To`nMeasure`n`nB - Locks`ncursor to`n H or V`naxis only
        ;;∙------∙Hide BLOCKED text if visible.
        If (blockedTextHwnd)
            GuiControl, Ruler: Hide, %blockedTextHwnd%
    }
    Else If (state = "awaiting_start") {
        If (blockedModeActive) {
            If (orientation = "Horizontal")
                GuiControl, Ruler:, MeasureDisplay, BLOCKED MODE: Press (Ctrl+Alt+LClick) to start
            Else
                GuiControl, Ruler:, MeasureDisplay, BLOCKED`nMODE`nPress`nCtrl+Alt`n+LClick
        } Else {
            If (orientation = "Horizontal")
    ...
    Else If (state = "awaiting_stop") {
        If (blockedModeActive) {
            If (orientation = "Horizontal")
                GuiControl, Ruler:, MeasureDisplay, Y-AXIS BLOCKED: Click again to complete measure
            Else
                GuiControl, Ruler:, MeasureDisplay, BLOCKED`n  H-AXIS`n   Click`nagain to`ncomplete`nmeasure
            ;;∙------∙Show the "BLOCKED" text on the ruler.
            If (blockedTextHwnd)
                GuiControl, Ruler: Show, %blockedTextHwnd%
        } Else {
            If (orientation = "Horizontal")
...
ToggleOrientation:
    ...
    measureBlockActive := false
    measureClickCount := 0
    ClipCursor()    ;;∙------∙Release cursor if blocked
    GoSub, BuildRulerGUI
Return
;;∙------------------------------------∙
ClearMeasurement:
