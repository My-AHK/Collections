...
;;∙======∙GUI-BUILD∙============∙
Gui, New, +AlwaysOnTop -Caption +hwndhGui +Owner +MinSize
Gui, Margin, 10, 15
Gui, Color, 0B0B0B
...
Gui, Add, Text, x285 y565 cLime BackgroundTrans gRefresh, Refresh
Gui, Add, Text, x300 y580 cRed  BackgroundTrans gExit, Exit
...
Update:
    ...
    UpdateText("Ctrl_Title"
        , t1 "`nahk_class " t2 "`nahk_exe " t3 "`nahk_pid " t4)
    ...
    UpdateText("Ctrl_MousePos"
        , "Screen:`t" msX ", " msY " (less often used)`n"
        . "Window:`t" mrX ", " mrY " (default)`n"
        . "Client:`t" mcX ", " mcY " (recommended)`n"
        . "Color:`t" mClr " (Red=" SubStr(mClr, 1, 2)
        . " Green=" SubStr(mClr, 3, 2)
        . " Blue=" SubStr(mClr, 5) ")")

    UpdateText("Ctrl_CtrlLabel"
        , (Ctrl_FollowMouse ? txtMouseCtrl : txtFocusCtrl) ":")
    ...
    UpdateText("Ctrl_Pos"
        , "`tx: " wX "`ty: " wY "`tw: " wW "`th: " wH
        . "`nClient:`tx: 0`ty: 0`tw: " wcW "`th: " wcH)
...
WinGetTextFast(detect_hidden)
{
    ...
    {
        if !detect_hidden
        && !DllCall("IsWindowVisible", "ptr", A_LoopField)
            continue

        if !DllCall("GetWindowText"
            , "ptr", A_LoopField
            , "str", buf
            , "int", WINDOW_TEXT_SIZE)
            continue
...
;;∙------∙MENU-HEADER∙---------------∙
Window-Spy_DarkMode:    ;;∙------∙Suspends hotkeys then pauses script.
    Suspend
    Soundbeep, 700, 100
    Pause
Return
