...
;;∙------∙Simulate Key Press (SendInput).
;;∙------∙Simulate pressing the 'B' key.
VK_B := 0x42    ;;∙------∙Virtual-key code for 'B'.
ScanCode := DllCall("MapVirtualKey", "UInt", VK_B, "UInt", 0)

VarSetCapacity(ki, 28, 0)
NumPut(1,   ki, 0,  "UInt")    ;;∙------∙INPUT type = keyboard.
NumPut(VK_B, ki, 4,  "UInt")    ;;∙------∙Virtual key.
NumPut(ScanCode, ki, 8,  "UInt")    ;;∙------∙Scan code.
NumPut(0,   ki, 12, "UInt")    ;;∙------∙dwFlags = 0 for key press.
NumPut(0,   ki, 16, "UInt")    ;;∙------∙Time.
NumPut(0,   ki, 20, "UPtr")    ;;∙------∙dwExtraInfo.
DllCall("User32\SendInput", "UInt", 1, "Ptr", &ki, "Int", 28)

NumPut(0x0002, ki, 12, "UInt")    ;;∙------∙Simulate key release (KEYEVENTF_KEYUP flag = 0x0002).
DllCall("User32\SendInput", "UInt", 1, "Ptr", &ki, "Int", 28)
;;∙------------------------------------------------------------------------------------------∙
...
;;∙------∙Retrieve the keyboard layout for the current thread.
threadID := DllCall("GetCurrentThreadId", "UInt")
layout := DllCall("User32\GetKeyboardLayout", "UInt", threadID, "Ptr")
MsgBox % "Current keyboard layout: 0x" Format("{:X}", layout)
;;∙------------------------------------------------------------------------------------------∙

;;∙------∙Get the Name of a Key.
;;∙------∙Convert a key’s scan code into its readable name.
;;∙------∙Get the readable name for the 'A' key (virtual-key code 0x41).
scancode := DllCall("MapVirtualKey", "UInt", 0x41, "UInt", 0)
lParam := scancode << 16  ; Build lParam with the scan code.
VarSetCapacity(keyName, 64)
DllCall("User32\GetKeyNameText", "Int", lParam, "Str", keyName, "Int", 64)
MsgBox % "Key name for 0x41: " keyName
;;∙------------------------------------------------------------------------------------------∙

;;∙------∙Retrieve the Current Keyboard State.
;;∙------∙Retrieve the state of all virtual keys.
VarSetCapacity(keyStates, 256, 0)
DllCall("User32\GetKeyboardState", "Ptr", &keyStates)    ;;∙------∙keyStates now contains 256 bytes representing each key's state.
MsgBox "Retrieved keyboard state for all keys."
;;∙------------------------------------------------------------------------------------------∙
...
;;∙===========================================∙
;;;;∙=======∙PROCESSES∙==================∙30 (split from SYSTEM)
...
;;∙------∙Determine the Number of Audio Output Devices.
numDevices := DllCall("winmm.dll\waveOutGetNumDevs")
MsgBox % "Number of Audio Output Devices: " numDevices
;;∙------------------------------------------------------------------------------------------∙

;;∙------∙Play a Sound File Using mciSendString (Asynchronously).
;;∙------∙Open and play an audio file via the Media Control Interface.
;;∙------∙Open an audio file with an alias and play it asynchronously.
;;∙------∙The alias (here, mySound) lets you refer to the sound later (e.g. to stop playback).
DllCall("winmm.dll\mciSendString", "Str", "open ""C:\Windows\Media\chimes.wav"" type waveaudio alias mySound", "Str", "", "UInt", 0, "Ptr", 0)
DllCall("winmm.dll\mciSendString", "Str", "play mySound", "Str", "", "UInt", 0, "Ptr", 0)
;;∙------------------------------------------------------------------------------------------∙

;;∙------∙Stop and Close a Sound Played via mciSendString.
;;∙------∙Stop and close the sound associated with the alias "mySound".
DllCall("winmm.dll\mciSendString", "Str", "stop mySound", "Str", "", "UInt", 0, "Ptr", 0)
DllCall("winmm.dll\mciSendString", "Str", "close mySound", "Str", "", "UInt", 0, "Ptr", 0)
;;∙------------------------------------------------------------------------------------------∙

;;∙------∙Retrieve the Current System Volume Levels.
;;∙------∙Retrieve the current volume settings for the default audio device.
;;∙------∙Returned DWORD contains the left channel in the low-order word and the right channel in the high-order word.
VarSetCapacity(vol, 4, 0)
DllCall("winmm.dll\waveOutGetVolume", "UInt", 0, "UInt*", vol)
;;∙------∙Extract left and right channel volumes (range 0–65535).
leftVol := NumGet(vol, 0, "UInt") & 0xFFFF
rightVol := NumGet(vol, 0, "UInt") >> 16
MsgBox % "Left Volume: " leftVol " | Right Volume: " rightVol
;;∙------------------------------------------------------------------------------------------∙
...
;;∙------∙SendMessage to Change a Window's Title.
;;∙------∙Retrieve the active window handle.
hwnd := DllCall("User32\GetForegroundWindow", "Ptr")
newTitle := "New Title via SendMessage"    ;;∙------∙New title to set.
DllCall("User32\SendMessage", "Ptr", hwnd, "UInt", 0x000C, "Ptr", 0, "Str", newTitle)    ;;∙------∙WM_SETTEXT = 0x000C.
MsgBox % "Active window title changed to: " newTitle
;;∙------------------------------------------------------------------------------------------∙
...
;;∙------∙PostMessage to Request Window Closure.
;;∙------∙Asynchronous and allows script to continue immediately.
hwnd := DllCall("User32\GetForegroundWindow", "Ptr")    ;;∙------∙Retrieve the active window handle.
DllCall("User32\PostMessage", "Ptr", hwnd, "UInt", 0x0010, "Ptr", 0, "Ptr", 0)    ;;∙------∙WM_CLOSE = 0x0010
MsgBox "WM_CLOSE posted to the active window."
;;∙------------------------------------------------------------------------------------------∙
...
;;∙------∙Manual Message Loop Using TranslateMessage & DispatchMessage - similar to OnMessage().
;;∙------∙Inspect, modify, or filter messages/Custom Message Filtering & Processing/
;;∙------∙Precise Loop Termination & Error Handling/Legacy or External Code Integration/Place to insert logging or debugging code.
;;∙------∙Allocate a buffer for a MSG structure (size may vary/48 bytes used here).
VarSetCapacity(msg, 48, 0)
while (DllCall("User32\GetMessage", "Ptr", &msg, "Ptr", 0, "UInt", 0, "UInt", 0) != 0) {    ;;∙------∙Loop until WM_QUIT is received.
    DllCall("User32\TranslateMessage", "Ptr", &msg)    ;;∙------∙Translate virtual-key messages to character messages.
    DllCall("User32\DispatchMessage", "Ptr", &msg)    ;;∙------∙Dispatch the message to the appropriate window procedure.
}
MsgBox "Message loop exited after receiving WM_QUIT."
;;∙------------------------------------------------------------------------------------------∙
