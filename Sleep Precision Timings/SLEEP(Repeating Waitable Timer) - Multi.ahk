...
^t::    ;;∙------∙🔥∙(Ctrl + T)
    Soundbeep, 1200, 250
    ;;∙------∙Each timer has its own handle and interval, and triggers a separate label.
...
StartPeriodicTimer(ms, labelName, ByRef hTimer) {
    ...
    interval := -ms * 10000  ;;∙------∙Convert ms to 100ns intervals (relative).
...
StopAllTimers() {
    ...
    Loop, Parse, TimerAHandle, `,
        If (A_LoopField)
            DllCall("CancelWaitableTimer", "Ptr", A_LoopField), DllCall("CloseHandle", "Ptr", A_LoopField)
    TimerAHandle := "", TimerBHandle := ""
}
Return
;;∙============================================================∙
;;∙============================================================∙
