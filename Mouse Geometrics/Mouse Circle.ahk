...
;;∙======∙Variables∙=================================∙
circleRadius := 200    ;;∙------∙Circle radius (pixels).
circleSpeed := 0.075    ;;∙------∙Movement speed (lower = faster).
isCircling := false    ;;∙------∙Track if currently circling.
...
^t::    ;;∙------∙🔥∙(Ctrl + T)
    if (isCircling) {
        isCircling := false    ;;∙------∙Stop.
        SetTimer, MoveInCircle, Off
        ToolTip, Circle movement stopped
        SetTimer, KillTip, 1000
    } else {
        GetCurrentMonitorCenter(centerX, centerY)
        isCircling := true    ;;∙------∙Start.
        angle := 0
        SetTimer, MoveInCircle, 10    ;;∙------∙Update every 10ms for smooth movement.
        direction := clockwise ? "clockwise" : "counter-clockwise"
        ToolTip, Circle movement started %direction% (Ctrl+T to stop`, Ctrl+R to reverse)
        SetTimer, KillTip, 1000
    }
Return
...
MoveInCircle:
    ...
    CoordMode, Mouse, Screen
    newX := centerX + (circleRadius * Cos(angle))
    newY := centerY + (circleRadius * Sin(angle))
    MouseMove, %newX%, %newY%, 0
    if (clockwise) {
        angle += circleSpeed
        if (angle >= 6.283185)    ;;∙------(2 * Pi)
            angle := 0
    } else {
        angle -= circleSpeed
        if (angle <= -6.283185)    ;;∙------(-2 * Pi)
