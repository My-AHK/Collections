...
;;∙======∙CONFIGURATION∙==(set actions here)===============∙
;;∙------∙Supports weekday filtering and action types: Delay, SoundBeep, Reload, ExitApp, and Run commands/URLs.
;;∙------∙First parameter can be a 3-character weekday (Sun, Mon, Tue, Wed, Thu, Fri, Sat) or omitted for daily execution.

TargetActions := {}
TargetActions["3:15 PM"] := ["SoundBeep"]    ;;∙------∙Daily (no weekday specified)
TargetActions["3:16 PM"] := ["Mon", "SoundBeep", "Delay:2000", "Reload"]    ;;∙------∙Monday only
TargetActions["3:17 PM"] := ["Fri", "notepad.exe", "Delay:1000", "SoundBeep"]    ;;∙------∙Friday only
TargetActions["3:18 PM"] := ["Tue", "SoundBeep", "Delay:1500", "SoundBeep", "Delay:1500", "https://www.google.com"]    ;;∙------∙Tuesday only
TargetActions["3:19 PM"] := ["Wed", "SoundBeep", "Delay:2500", "ExitApp"]    ;;∙------∙Wednesday only
TargetActions["3:20 PM"] := ["Sun", "SoundBeep"]    ;;∙------∙Sunday only
...
Check_Time:
    ...
    FormatTime, CurrentWeekday, , WDay    ;;∙------∙Get current weekday (1=Sunday, 7=Saturday).
    ...
    if TargetActions.HasKey(FormattedTime)
    {
        ...
        {
            actions := TargetActions[FormattedTime]
            
            ;;∙------∙Check if first parameter is a weekday filter.
            if (actions.Count() > 0)
            {
                firstParam := actions[1]
                validWeekdays := ["Sun", "Mon", "Tue", "Wed", "Thu", "Fri", "Sat"]
                isWeekdayFilter := false
                
                for index, day in validWeekdays
                {
                    if (firstParam = day)
                    {
                        isWeekdayFilter := true
                        break
                    }
                }
                
                ;;∙------∙If weekday filter exists, check if it matches current day.
                if (isWeekdayFilter)
                {
                    if (firstParam != CurrentWeekdayStr)
                    {
                        Log_Entry("Skipped " FormattedTime " - weekday filter (" firstParam ") does not match current day (" CurrentWeekdayStr ")")
                        return
                    }
                    ;;∙------∙Remove weekday filter from actions before execution.
                    actualActions := []
                    Loop % actions.Count() - 1
                        actualActions.Push(actions[A_Index + 1])
                }
                else
                {
                    ;;∙------∙No weekday filter, use all actions.
            ...
            Save_Flags()
            Log_Entry("Triggered at " FormattedTime " on " CurrentWeekdayStr)
            
            for index, act in actualActions
                Perform_Action(act)
        }
    }
Return
...
Validate_Configuration()
{
    ...
    for timeStr, actions in TargetActions
    {
        ...
        ;;∙------∙Check if first parameter is weekday filter.
        firstParam := actions[1]
        hasWeekdayFilter := false
        if (firstParam != "")
        {
            if Validate_Weekday(firstParam)
            {
                hasWeekdayFilter := true
                Log_Entry("Info: " timeStr " has weekday filter: " firstParam)
                ;;∙------∙Check if there are actions after the weekday filter.
                if (actions.Count() = 1)
                {
                    Log_Entry("Warning: " timeStr " has weekday filter but no actions defined.")
                    continue
                }
            }
        }

        ;;∙------∙Validate individual actions (skip first if it's weekday filter).
        startIndex := hasWeekdayFilter ? 2 : 1
        Loop % actions.Count()
        {
            ...
            if (action = "")
            {
                Log_Entry("Warning: Empty action found for " timeStr " at index " A_Index)
            }
