...
global Reminder_GUI_Count := 0
global globalReminderGUIs := []
...
;;∙======∙(0= Edge+10, 1 = 1/4, 2 = 1/2, 3 = 3/4 of screen).
...
ShowCalendar() {
    ...
    Gui, Calendar:Add, Button, x170 yp-10 w60 gSetReminder, Schedule
    Gui, Calendar:Add, Button, x+5 w60 gCancelCalendar, Cancel

    Gui, Calendar:Add, Picture, x235 y+15 w16 h16 gHide Icon248, shell32.dll
    Gui, Calendar:Add, Picture, x+7 w16 h16 gRefresh Icon239, shell32.dll
    Gui, Calendar:Add, Picture, x+7 w16 h16 gExit Icon132, shell32.dll
...
;;∙===============================================∙
UpdateCalendarTime:
    if (WinExist("ahk_id " . CalendarHwnd)) {
        FormatTime, CurrentDate,, dddd`, MMM. dd, yyyy
        FormatTime, CurrentTime24,, HH:mm:ss  tt
...
ShowErrorGui(errorMessage) {
    ...
    Gui, Error:Add, Button, x%buttonX% y+20 w60 gCloseErrorGui Default, OK
    Gui, Error:Add, Text
...
ShowPastDateWarning(month, day, year, time, reminderText) {
    ...
    Gui, PastWarning:Add, Button, x+%buttonSpacing% w%buttonWidth% gCancelPastDate Default, No
...
ShowSuccessGui(reminderText, timestamp) {
    ...
    Gui, Success:Add, Text, x20 y+10 w%textWidth% Center, Scheduled for:`t%scheduledFormatted%
    Gui, Success:Add, Text, x20 y+5 w%textWidth% Center, Current time:`t%currentTimeFormatted%

    buttonX := (guiWidth // 2) - 30
    Gui, Success:Font, s9 c%guiColor%, Segoe UI
    Gui, Success:Add, Button, x%buttonX% y+15 w60 gCloseSuccessGui Default, OK
...
CheckDueReminders() {
    ...
    for index, reminder in remindersToShow {
        ShowReminder(reminder)
        for i, r in reminders {
            if (r.timestamp = reminder.timestamp && r.text = reminder.text) {
...
;;∙===============================================∙
ShowReminder(reminder) {
    global Reminder_X_Quarter, Reminder_Y_Quarter, Reminder_GUI_Count, globalReminderGUIs, LastReminderTime

    currentTime := A_TickCount
    timeSinceLast := currentTime - LastReminderTime

    if (timeSinceLast < 1000) {
        Sleep, 1000 - timeSinceLast
    }
    
    LastReminderTime := A_TickCount
    ...
    guiName := "ReminderGUI_" . timestamp . "_" . Reminder_GUI_Count

    pos := GetGuiPosition(Reminder_X_Quarter, Reminder_Y_Quarter, guiWidth, guiHeight)
    guiX := pos.x
    guiY := pos.y

    if (!IsObject(globalReminderGUIs))
        globalReminderGUIs := []
    globalReminderGUIs.Push(guiName)
    ...
    Gui, %guiName%:Add, Text, x10 y+10 w300 Center, Scheduled For:`t%scheduledFormatted%
    Gui, %guiName%:Add, Text, x10 y+5 w300 Center, Presented At:`t%currentTimeFormatted%

    Gui, %guiName%:Font, s9 c%guiColor%, Segoe UI
    Gui, %guiName%:Add, Button, x140 y+15 w60 gCloseReminder Default, OK
    Gui, %guiName%:Add, Text

    Gui, %guiName%:Show, x%guiX% y%guiY% w330, Reminder - %guiName%
    GuiControl, Focus, Button1
    return
}
...
CloseReminder:
    ...
    if (InStr(windowTitle, "ReminderGUI_")) {
        guiName := ""
        if (RegExMatch(windowTitle, "ReminderGUI_\d+_\d+_\d+", match)) {
            guiName := match
        } else {
            StringReplace, guiName, windowTitle, Reminder - ,, All
            guiName := Trim(guiName)
        }
        ...
        if (guiName) {
            ...
            if (IsObject(globalReminderGUIs)) {
                newArray := []
                for index, name in globalReminderGUIs {
                    if (name != guiName) {
                        newArray.Push(name)
                    }
                }
                globalReminderGUIs := newArray
            }
            return
        }
    }
...
ExitFunc(ExitReason, ExitCode) {
    ...
    if (IsObject(globalReminderGUIs)) {
        for index, guiName in globalReminderGUIs {
            Gui, %guiName%:Destroy
        }
    }
}
