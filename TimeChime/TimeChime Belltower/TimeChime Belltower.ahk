...
;;∙------∙Chime Settings.
HourlySoundFile := "HOUR.mp3"    ;;∙------∙File for hourly chime sound.
HalfHourlySoundFile := "HALF HOUR.mp3"    ;;∙------∙File for 1/2 hour chime sound.
QuarterHourlySoundFile := "15 & 45 MINS.mp3"    ;;∙------∙File for 15 & 45-minute chime sound.
AlarmSoundFile := "ALARM.mp3"    ;;∙------∙File for alarm sound.
...
AlarmRepeat := 3    ;;∙------∙Number of consecutive sounds for alarm.

;;∙------∙Timing Setting.
AlertWindow := 15    ;;∙------∙Time (in seconds) after minute starts during which alerts are allowed.

;;∙------∙ToolTip Removal Time.
TtRT := 3500    ;;∙------∙Time (in milliseconds) to display ToolTips.
...
LastSoundMinute := -1    ;;∙------∙Initialize variable to track last minute a chime was played.
LastAlarmDate := "none"    ;;∙------∙Initialize variable to track last date alarm was triggered.

;;∙------∙Hour + minute tracking variables.
LastSoundTime := "none"    ;;∙------∙Tracks HHMM format for accurate hourly checks.
LastSoundDate := "none"    ;;∙------∙Handles midnight reset.
Return
...
if (EnableTestHotkeys)    ;;∙------∙Check if testing hotkeys are enabled.
{
    ...
    ^!1::
        ...
        SoundPlay, % HourlySoundFile, WAIT
    Return
    ...
    ^!2::
        ...
        SetTimer, ClearTip, % -TtRT
        SoundPlay, % HalfHourlySoundFile, WAIT
    Return
    ...
    ^!3::
        ...
        SetTimer, ClearTip, % -TtRT
        SoundPlay, % QuarterHourlySoundFile, WAIT
    Return
    ...
    ^!4::
        ...
        SetTimer, ClearTip, % -TtRT
        Loop % AlarmRepeat {
            SoundPlay, % AlarmSoundFile, WAIT
            Sleep 150
        }
    return
}

CheckTime:    ;;∙------∙Get padded time components.
    ...
    CurrentTimeKey := CurrentHour . CurrentMinute    ;;∙------∙Create unique time identifier (HHMM format).

    if (CurrentDate != LastSoundDate) {    ;;∙------∙Midnight reset.
        LastSoundTime := "none"
        LastSoundDate := CurrentDate
    }
    ...
    if CurrentMinute in 00,15,30,45    ;;∙------∙Padded string format.
    {
        ...
        if (CurrentTimeKey != LastSoundTime)    ;;∙------∙Ensure chime is only played once per hour + minute.
        {
            ...
            LastSoundTime := CurrentTimeKey    ;;∙------∙Update tracking with full time.
        }
    }
    else
    {
        LastSoundTime := "none"    ;;∙------∙Reset outside chime minutes.
    }
...
;;∙========∙ALARM SECTION∙=================================∙
    ...
    if (AlarmEnabled)    ;;∙------∙Check if alarm is enabled.
    {
        ...
        if (A_Hour = AlarmHour && A_Min = AlarmMinute && !AlarmTriggered)
        {
            ...
            SetTimer, ClearTip, % -TtRT
            ;;∙------∙Loop to play alarm sound multiple times.
            Loop % AlarmRepeat
            {
                SoundPlay, % AlarmSoundFile, WAIT    ;;∙------∙Play alarm sound.
                Sleep 150    ;;∙------∙Pause for 150ms between sounds.
            }
