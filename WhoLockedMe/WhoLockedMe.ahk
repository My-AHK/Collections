...
» Original Source:  https://www.autohotkey.com/boards/viewtopic.php?f=6&t=15857#p80455
» GitHub Source :  https://github.com/T-vK/WhoLockedMe/blob/master/WhoLockedMe.ahk
∙=============================================================∙
*/
;;∙------------------------------------------------------------------------------------------∙
;;∙======∙Auto-Execute∙==========================================∙
ScriptID := "WhoLockedMe"    ;;∙------∙Also change in 'MENU CALLS' at script end.
GoSub, AutoExecute
GoSub, TrayMenu
;;∙============================================================∙
...
Gui_FileHandles_CtrlEvt(CtrlHwnd:=0, GuiEvent:="", EventInfo:="", ErrLvl:="") {
    ...
    If (ControlName = "Gui_FileHandles_ApplyFilter") {
        ...
        Loop {
            ...
            HandleClosed := DuplicateObject(ProcHandle, 0, Handle, 0x1)
            If HandleClosed {
...
GetIconByPath(Path, FileExists:="") {    ;;∙------∙Fully qualified file path, result of FileExist on Path (optional).
    ...
    } Else    ;;∙------∙If the file is deleted return an appropriate icon. 
        Return LoadPicture(A_WinDir "\System32\imageres.dll", "Icon85 w16 h16", IconType)    
    VarSetCapacity(SFI, cbSFI, 0)
...
;;∙------------------------------------------------------------------------------------------∙
;;∙======∙MENU CALLS∙==========================================∙
WhoLockedMe:    ;;∙------∙Suspends hotkeys then pauses script. (Script Header)
    Suspend
    Soundbeep, 700, 100
    Pause
Return
