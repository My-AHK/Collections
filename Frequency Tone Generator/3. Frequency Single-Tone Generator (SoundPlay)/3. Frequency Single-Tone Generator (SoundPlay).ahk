...
Gui, Add, Text, x10 y20 BackgroundTrans, Frequency
Gui, Font, Norm
Gui, Add, Edit, x85 yp c3D9EFF vFreqEdit w60 h20, 1100
Gui, Font, s8
Gui, Add, Text, x+3 yp-3 BackgroundTrans, (Hz)

Gui, Font, Italic
Gui, Add, Button, gPlayToneSection x+10 yp+3 w45 h20, Play

Gui, Font, s10 c3D9EFF Bold Q5, Arial
Gui, Add, Text, x10 y60 BackgroundTrans, Duration
Gui, Font, Norm
Gui, Add, Edit, x85 yp c3D9EFF vDurEdit w60 h20, 750
Gui, Font, s8
Gui, Add, Text, x+3 yp-3 BackgroundTrans, (ms)

Gui, Font, Italic
Gui, Add, Button, gExitScript x+10 yp+3 w45 h20, Exit
Gui, Show, x1550 y450 w240 h100, Frequency Tone Generator
Return
...
PlayToneSection:
    Gui, Submit, NoHide
    if (FreqEdit < 37 or FreqEdit > 32767)
    {
        MsgBox, 48, Error, Frequency must be between 37 and 32767 Hz., 4
        Return
    }
    if (DurEdit < 1)
    {
    ...
    ;;∙------∙Create temporary WAV file with specified frequency and duration.
    TempFile := A_Temp "\TempTone_" A_Now ".wav"
    CreateToneWav(FreqEdit, DurEdit, TempFile)
...
CreateToneWav(Frequency, Duration, FileName) {
    ;;∙------∙WAV File Parameters.
    SampleRate := 44100
    BitsPerSample := 16
    NumChannels := 1
    ...
    Amplitude := 0.4    ;;∙------∙Reduce To Avoid Clipping When Mixed.
    AngularFreq := 2 * 3.141592653589793 * Frequency / SampleRate
    Samples := Floor(SampleRate * Duration / 1000)

    Loop %Samples% {
        Sample := Amplitude * Sin(AngularFreq * (A_Index - 1))
        ;;∙------∙Convert To 16-Bit Signed Integer.
        IntSample := Round(Sample * 32767)
